...
//	Tree* l = left_count==0?NULL:tree_from_preorder_and_inorder(preorder.substr(1, left_count), inorder.substr(0, left_count));
//	Tree* r = left_count==preorder.length()-1?NULL:tree_from_preorder_and_inorder(preorder.substr(left_count+1), inorder.substr(left_count+1));
//	return new Tree(l, value, r);
//}
//
...
//	for(auto it:freqs)
//		q.push(new HuffmanTree(it.first, it.second));
//	while(q.size()>1){
